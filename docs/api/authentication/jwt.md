...
A payload **require** a user id (*user_id*), an expiration time (*exp*), an issued at a time (*iat*) and a JWT ID (*jti*).
The *ext_id* is not necessary to authenticate the user.
