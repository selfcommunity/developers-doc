---
sidebar_label: Remove a Tag
sidebar_position: 1
title: Remove a Tag
---

This endpoint removes a tag to a user or to a category or to a contribution.
...
If the specified object does not already have this tag associated, the operation is a no-op: nothing changes, and the
API still returns 204 No Content (no error).

::::

## HTTP Request

`POST /api/v2/tag/{id}/remove/`
...
curl -X POST /api/v2/tag/{id}/remove/ \
